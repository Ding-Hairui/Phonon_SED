...
# the atomic velocity matrix may be too large to load into GPU at once
# and the intermediate data generated by numpy/cupy when calculating also take up some memory
# so we estimate a max matrix size that our GPU can handle (1080 Ti, with 11264MiB memory)
# and slice the original matrix into several small matrixes
gpu = int(device[-1])
t_size = int(11264/4*1024*1024/3/n_atoms / 6)
n_load = int(np.ceil(total_frames / t_size))
t_idx = [i for i in range(n_load)]
...
for k in tqdm(range(nkp)):
	...
	with cp.cuda.Device(cp.cuda.Device(gpu)):
		expr = cp.array(expr)
		for i in t_idx:
			vtmp = cp.array(v[i*t_size : (i+1)*t_size]) * expr
			va = 1j * cp.zeros([vtmp.shape[0], mass.shape[0], 3])
