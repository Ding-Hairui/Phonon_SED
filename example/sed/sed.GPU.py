...
device = 'gpu0' if len(sys.argv) < 3 else sys.argv[2]
...
expr = np.exp(1j * (kpath_all.reshape([nkp, 1, 3]) * rl.reshape(1, n_atoms, 3)).sum(axis=2)).reshape([nkp, n_atoms, 1])
gpu = int(device[-1])
with cp.cuda.Device(cp.cuda.Device(gpu)):
	expt = cp.array(expt)
	expr = cp.array(expr)
	sed = cp.zeros([n_freq, nkp])
	vk = 1j * cp.zeros([n_freq, nkp, mass.shape[0], 3])
	mass = cp.array(mass)
	atoms_map = cp.array(atoms_map)
	for it in tqdm(range(total_frames)):
		vt = cp.array(v[it])
		for ik in range(nkp):
			vtmp = vt * expr[ik]
			va = 1j * cp.zeros([1, mass.shape[0], 3])
			for j in range(mass.shape[0]):
				va[0, j, :] = vtmp[atoms_map[j], :].sum(axis=0)
			va = va * expt[:, it] * dt
			vk[:, ik] += va
			tmp = cp.abs(vk[:, ik]) ** 2
			tmp = (tmp * mass).sum(axis=1).sum(axis=1) / 4 / cp.pi / (it+1) / n_sc
			sed[:, ik] = tmp

sed *= unit
cp.save(out_file, sed)
